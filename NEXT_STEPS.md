...
7. **Сверка UI с макетом + персист истории диалога** — 7b сделано 06.09.2026,
   ...
   - **7c. Персист истории диалога через несколько прогонов — TODO (backend).**
     ...
     последний обмен, предыдущие туры пропадают. Нужно свернуть проекции всех
     прогонов треда по порядку (или писать кумулятивную проекцию треда) — это
     граница AG-04/AG-07, не UI. Стриминг активностей: `activities` в проекции
     тоже по одному прогону; «Что проверено» после reload показывает только
     последний.
     - Не сделано и намеренно оставлено на потом: metric-cards grid и
       year-column раскладка финансов из `pReport` (крупная переразметка
       `LiveCompanyReport`); i18n `case_status_raw` («Истец · Appealed»).
