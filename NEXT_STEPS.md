...
## Локальный запуск на Mac — 06.09.2026

По запросу пользователя установлен Docker Desktop в `/Applications/Docker.app`;
`docker` и Compose доступны из терминала. `make up` успешно собрал все образы,
выполнил миграции/импорт и поднял стек. Для этого запуска опубликованные порты
привязаны к `127.0.0.1` через переменные окружения; Compose и `.env` не менялись.
`make smoke`: proxy/ui_api/agent/mcp — HTTP 200, все постоянные контейнеры healthy.
`make seed` создал проект `e3c139f5-a4bc-4816-9278-fa7640b9bbda` с двумя компаниями.
В браузере проверены демо-вход, загрузка проекта и отчёта с новым оформлением.
Приложение оставлено работающим на `http://localhost:5173`; это проверка запуска,
не новая приёмка качества ответов модели. Остановка без удаления БД — `make down`.

## Визуализация web без перестановки блоков — 06.09.2026

По запросу пользователя работаем с решением коллеги в `dev`: расположение
экранов, чат, API и агент не переделываем. В существующий раздел финансов
добавлены Recharts-графики с выбором показателя и вида (линия/столбцы), в
сравнение — столбцы компаний за сопоставимый период. Графики читают уже
загруженные DTO, не вызывают модель и открывают прежние evidence callbacks.
Пропуски остаются пропусками, ноль отличается от них, валюты/масштабы не
смешиваются. Точные суммы берутся из Decimal-строк, не из координат графика.

Прибыль добавлена внутрь существующей краткой финансовой карточки: плюс —
зелёный, минус — красный, ноль/неизвестное — нейтральные. Такое же отображение
в таблицах; это знак суммы, не оценка риска. Даты переведены в русский формат,
моменты — в часовой пояс браузера, сообщения и сохранённые действия показывают
дату и время. Календарные даты без времени не сдвигаются. Уточнены стили карточек,
таблиц и composer; исправлено горизонтальное переполнение таблицы на mobile.

Проверено: typecheck/build; 21 новая проверка форматирования/графиков.
В браузере проверены реальные компоненты на исходном JSON: широкая и узкая
поверхности (390/1024 px), переключатели, знаки прибыли, источник точки.
Это визуальная проверка, не сквозная приёмка: backend здесь не запущен.
Исходный HEAD отдельно проверен во временной копии: те же 46 старых падений
тестов и 2 ошибки lint сохраняются; новых падений общего набора нет
(после изменений 82 passed / 46 failed против 61 passed / 46 failed в HEAD).
Прежние ошибки относятся в том числе к отсутствующему scrollIntoView в jsdom,
устаревшим сценариям и mock-ответу summary. Их исправление вне этого UI-среза.

## Текст для финальной защиты — 06.09.2026

По запросу пользователя подготовлен `artifacts/solution-description-final.md`:
7 абзацев аннотации, проблематика, задача, техническое решение, результаты,
планы и команда. Сохраняемый разговор — основной сценарий; документы, отдельный
ИИ-артефакт и внешний MCP отмечены как дальнейшее развитие. Имена, роли и высокая
степень участия взяты из присланной промежуточной версии. Код не менялся;
описание не является новой приёмкой MVP или подтверждением экономии времени.
По следующему замечанию пользователя текст сокращён, убраны дата состояния,
канцелярские формулировки и повторы. Сохранены обязательные разделы и 7 абзацев
аннотации; дальнейшие правки делать кратко и простым языком.

## Тесты Agent Service — решение пользователя 06.09.2026

В `services/agent` оставляем общие устойчивые проверки инфраструктуры,
контрактов, конфигурации, изоляции, evidence refs и безопасности. Удаляем
хрупкие проверки конкретных ответов, промптов, форматирования, подбора заметок
и сценарных действий модели. Новые такие автотесты не писать и удалённые
не восстанавливать, в том числе в eval-наборах и на deterministic моделях.
Поведение проверяется ручным живым прогоном при необходимости. Это решение
заменяет прежние требования к автоматическим agent evals в очереди ниже;
исторические результаты тестов остаются историей.

Удалены 28 функций хрупких тестов; в проверках provisioning убраны привязки
к точным фразам. Оставшиеся проверки: 53 passed, 14 PostgreSQL checks skipped
без `AGENT_TEST_POSTGRES_DSN` / `AGENT_TEST_RUNTIME_DSN`; ruff, format и mypy пройдены.
...
## Экран отчёта — 06.09.2026

По запросу пользователя меняется только вёрстка отчёта под дизайнерский HTML:
карточки, открытые разделы, финансовая таблица по годам, компактные основания
и «Обсудить». API, выбор компании и evidence сохраняются. Тесты по просьбе
пользователя не добавлялись; production build проверен. Текст AI из макета
не подставляется вместо реального вывода.
...
7. **Сверка UI с макетом + персист истории диалога** — 7b сделано 06.09.2026,
   ...
   - **7e. Сильная модель + подгонка агента под demo test pack (`DEMO.md`).** —
     заход 06.09.2026.
     - **Модель:** переключение на GLM через NeuralDeep (тот же ключ/base URL,
       модель есть в `/v1/models`). `glm-5.2` умнее, но ~250s на ход (reasoning
       + tool-round-trips) — для живого демо слишком медленно; остановился на
       `glm-5.3-flash`. `.env` (не в git): `AGENT_MODEL_ID=glm-5.3-flash`,
       `AGENT_MODEL_MAX_TOKENS=8192`, `AGENT_MAX_TOOL_CALLS=6`,
       `AGENT_RUN_TIMEOUT_SECONDS=240`.
     - **Профиль клиента** (`DEMO.md` §2): `harness/client_profile.py` —
       `ClientProfile` + `DEMO_CLIENT` (ООО «СтройКонтур», строительство, малый
       бизнес, гендиректор, РКО+Альфа-Про). Рендерится в системный промпт как
       фон; условия сделки агент по-прежнему узнаёт из диалога.
       `AGENT_CLIENT_PROFILE_JSON` — override. `deterministic._find_inn` теперь
       пропускает `SystemMessage` (в промпте появился ИНН клиента).
     - **Промпт → разговорный формат** под ожидаемые ответы `DEMO.md`: 4–7
       предложений без списков-шаблонов, суть под задачу первым предложением,
       2–3 факта, динамика одной строкой «2024 → 2025», последнее предложение —
       один вопрос/следующий шаг. Отдельные режимы: сильный сигнал (высокий
       риск/оплата завтра) — коротко, «не совершайте необратимое действие»,
       без «ненадёжный»; сравнение — не места 1-2-3, а какой вопрос закрыть по
       каждой; «нет раздела» ≠ «нет риска». Бюджет обращений: обычно overview +
       finances, лишние разделы не открывать.
     - **ЗСК в словаре** смягчён: показывать значение как пришло, шкалу не
       навязывать (в mock-данных ЗСК — цвет, а не низкий/средний/высокий).
     - **`repair_answer`:** если после отбраковки осталось ≥3 grounded-факта —
       выкидываем ненайденные строки молча, без пугающего блока «Неизвестно:
       часть утверждений исключена». Блок остаётся, только когда ответ реально
       обеднён. +тест.
     - Проверки: agent ruff/format/mypy + 85 passed / 14 skipped.
     - Тест по сценариям `DEMO.md` — прогон через RPC (минуя UI), результаты
       в отчёте пользователю.
...
- Внешний MCP/export и whitelist полей; основной MVP использует внутренний MCP.
   ...
   - **7f. Прогон demo test pack + модель + UI-доводка.** — заход 06.09.2026.
     - **Модель:** GLM-5.2/5.3-flash через harness слишком медленны (reasoning ×
       tool-round-trips = 4 мин/ход, упираются в таймаут). Демо-модель —
       `deepseek-v4-pro` (`.env`, не в git): полные ответы, короткая проза,
       ~100–110 c/ход. `AGENT_MAX_TOOL_CALLS` теперь пробрасывается через compose
       (был захардкожен на 12) — стоит 5, срезал ход почти вдвое.
     - **Прогон 3 сценариев DEMO.md через RPC (минуя UI):**
       - С1 (ТЕТРАДОМ, golden): суть — «риск низкий, но для аванса важнее
         финансы»; отрицательный капитал −5.3М → −29.6М, убыток 28.6М, «своих
         средств нет»; активы vs дебиторка; массовый директор с верной рамкой;
         закрывает вопросом «какая сумма и срок?». ✅
       - С2 (сравнение 3): начать с ТЕХПРОФ (единственный + капитал), у ТЕТРАДОМ
         разобрать разрыв активы/капитал, у КВАЛИС «меньше всего данных»
         (жёлтый + пустая отчётность). Различает «нет плохого» и «мало данных». ✅
       - С3 (ТЕХПРОМ, HIGH): лидирует сигналом (светофор высокий + ЗСК RED),
         свежая регистрация, прибыль 1000 ₽ при выручке 96М; на «цена ниже,
         рискнуть?» — «скидка не перекрывает два красных сигнала», без «ненадёжен»
         и без запрета. ✅
       - Осталось нестрого: местами модель дробит предложение на строки
         (частично лечит `_tidy_answer`), список «что запросить» не всегда даёт.
     - **UI-правки по скриншоту пользователя:**
       - Лог действий больше не исчезает после ответа: во время прогона — текущая
         строка (running-шаг или последний, пока модель думает между тулами, без
         мигания) и список строится на месте; после — сворачивается под ответ за
         «Что проверено». Строки «Проверка завершена» нет.
       - Компанию, которую агент закрепил по ИНН (`add_company_to_check`), UI
         теперь показывает без перезагрузки: `onRunSettled` → invalidate запроса
         проекта → `CompanyContextStrip`. Проверено сквозняком: пустой проект →
         «Проверь 9714038662…» → `companies:['9714038662'] ctx_v:1`.
     - **client_profile** (7e) уже в промпте — ООО «СтройКонтур», строительство.
