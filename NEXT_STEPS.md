...
6. **Быстрый рефакторинг раскладки (только перемещение файлов)** — `services/ui_api/src/counterparty_ui_api/`
   ...
   импорты, прогнать тесты. Поведение не меняется, отдельным коммитом. По
   желанию — сгруппировать `report_*` в `packages/domain`. `agent`, `storage`,
   `web` уже структурированы, их не трогать.

Независимый проверяющий — только на п. 2 и живой прогон п. 5.

**Вне этого захода** (отдельная сессия после каркаса): AG-06 (справочник знаний
агента, Specs 04 §6) и QA-04 (агентные evals). Заведены в WORK_PLAN. Пользователь
берёт на себя дожимание агентского сценария и UI/UX после MVP-каркаса.
