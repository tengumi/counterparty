...
7. **Сверка UI с макетом + персист истории диалога** — 7b сделано 06.09.2026,
   ...
   - Не сделано и намеренно оставлено на потом: metric-cards grid и year-column
     ...
     i18n `case_status_raw` («Истец · Appealed»); агент сам добавляет компанию
     по ИНН из сообщения (сейчас просит нажать «Добавить») — нужна запись в
     workspace на стороне агента, дублирует provisioning ui_api.
