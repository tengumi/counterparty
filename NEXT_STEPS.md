...
**Текущая цель:** непрерывно завершить рабочий MVP по Specs: интегрировать J,
закрыть persistence и REST-стыки, довести агентный сценарий и общую приёмку.
Срез I принят 05.09.2026; пользователь разрешил завершение без промежуточных
согласований волн.

## Финализация — решение пользователя 05.09.2026

«Удали лишние, заверши старые; нужно всё доделать по спекам, чтобы работало и
был классный агентский сценарий». Работа продолжается укрупнёнными этапами:

1. Удалить чистые интегрированные E/F/G worktrees; сохранить незавершённые J.
2. Завершить J1/J2/J3 и интегрировать в общий baseline. Незакоммиченный J3
   сохранить и продолжить, не переписывать с нуля.
3. Закрыть AG-04 и недостающие REST conversation/artifacts/decisions:
   реальный scoped context, история, reconnect/cancel/restart, запись решения.
4. Добавить AG-06 и QA-04: знания о корректной интерпретации отчёта,
   grounded полезный ответ и проверки поведения; независимо проверить AG-03/04.
5. Собрать Compose, выполнить интеграционную и итоговую браузерную приёмку,
   подготовить demo runbook и точный остаток ограничений.

Сохраняется принятая граница MVP (документы и follow-up — post-MVP), если
пользователь не уточнит её иначе. Провайдер/модель подключаются конфигурацией;
секреты не требуются для детерминированной приёмки. Готовность реальной модели
не объявляется без соответствующего прогона.
...
## Текущий срез J: агентный harness, сравнение и общий контур

Решение пользователя 05.09.2026 о сокращении: MVP ограничен сквозной историей
«агент отвечает по закреплённому отчёту с проверяемыми evidence refs». DOC-01…03,
AG-05 и WEB-10 перенесены в post-MVP. Независимые проверяющие агенты назначаются
только на AG-03 и AG-04. Полная формулировка границы — в
[`docs/WORK_PLAN.md`](docs/WORK_PLAN.md) §5.

Решение пользователя 05.09.2026: шаги укрупняются, дробные подходы и
промежуточные checkpoints на MVP не оправданы. Оставшийся путь — две волны и
пять субагентов вместо трёх волн и девяти.

| Волна | Задачи | Исполнение |
|---|---|---|
| J | AG-01+AG-02+AG-03 · MCP-03+OPS-01 · WEB-09+WEB-11 | три агента параллельно |
| K | AG-04 · независимая проверка grounding и рантайма · сквозная приёмка | два агента, REL-01 у главного |

Уточнение по harness, внесённое до старта J: Deep Agents — агентный harness,
LangGraph отвечает за состояние и исполнение, LangChain — за провайдеров моделей.
Собственный agent loop и собственный роутер вызова инструментов не пишутся
(Specs 01 §32, Specs 08). `deepagents` ещё не объявлен в зависимостях
`services/agent`; это делает J1.

| Task | Статус | Ownership |
|---|---|---|
| J1 / AG-01+02+03 | in progress | `services/agent/src/**` и его тесты; `services/agent/pyproject.toml` |
| J2 / MCP-03+OPS-01 | in progress | `services/mcp/**`, `compose.yaml`, все `Dockerfile`, раздел запуска в README |
| J3 / WEB-09+11 | in progress | `apps/web/**` |

Границы между J1 и J2: Dockerfile сервиса agent принадлежит J2, исходники и
зависимости — J1. Если Docker build требует правки `pyproject`, J2 останавливается
и сообщает главному агенту. `packages/**` read-only для всех троих: transport DTO
C-03 и evidence-контракты уже зафиксированы, изменение идёт через главного агента.
J3 строит транспорт против существующего публичного контракта
`services/agent/src/counterparty_agent/transport/**`, не дожидаясь AG-04.

Provider/model: пользователь подключает модель сам. J и K строятся на
deterministic adapter, model ID не хардкодится; конфигурация провайдера остаётся
точкой расширения, а не задачей волны.

Решение пользователя 05.09.2026 по качеству агента: сначала закончить остальное,
затем добавить справочник знаний (AG-06) и агентные evals (QA-04). Обе задачи
заведены в WORK_PLAN и идут после J, в объём волны не входят. Пользователь берёт
на себя дожимание агентского сценария и UI/UX после MVP-каркаса.
