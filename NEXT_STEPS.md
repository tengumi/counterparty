...
## Оформление строки компаний — 06.09.2026

По скриншоту пользователя сделать название проверки и строку компаний единым
компактным блоком: нейтральная плашка компании, одинаковая высота действий,
спокойная кнопка добавления и отдельный акцент сравнения. Без новых подписей,
статусов и изменения действий. На узком экране кнопки переносятся без обрезания.
Реализовано: нейтральные плашки с иконкой, кнопки высотой 44 px, единый белый
фон без линии между названием и компаниями. Длинное имя сокращается визуально,
полное остаётся в доступном имени и подсказке. В браузере проверены вид на 1280 px,
отсутствие переполнения и открытие списка компаний; web пересобран.
Typecheck, production build и 10 профильных тестов пройдены. Общий набор:
92 passed / те же 43 failed (имена сверены), 2 прежние ошибки исполнения;
lint — те же 2 сторонние ошибки. Данные и поведение агента не менялись.
По выбору пользователя портфель заменён домиком из той же библиотеки Alfa;
размер, нейтральный цвет и поведение плашки сохранены.

## Упрощение шапки — решение пользователя 06.09.2026

Убрать «В работе» из истории и строки компаний, переключатель «Чат проверки»
и подтверждение «Сохранено» под названием. Ошибки сохранения с повтором,
сохранённые разговоры и остальные статусы остаются. Заголовок и описание S1
выровнять по левому краю блока ввода; сам блок остаётся в центре страницы.
Это уточнение прежней композиции шапки; серверное сохранение не менять.
Реализовано. Удалены неиспользуемый переключатель и его стили; сохранённые
разговоры доступны по прежним ссылкам, переименование и повтор сохранения работают.
В браузере проверены главная и сохранённая проверка: общий левый край,
отсутствие трёх подписей/кнопки и горизонтального переполнения. Web пересобран,
локальное приложение запущено. Typecheck, production build и 16 профильных
тестов пройдены. В полном наборе 91 passed / 43 failed, 2 ошибки исполнения;
старые проверки удалённого переключателя заменены проверкой компактной шапки.
Остаются прежние ошибки общего тестового окружения и 2 сторонние ошибки lint.

## Читаемость отчёта в «Материалах» — 06.09.2026

По скриншоту пользователя исправлено сжатие карточек в панели 400 px:
сетка и краткая справка зависят от ширины отчёта, а не окна браузера.
Узкая панель — одна колонка, средняя область — две, широкий отчёт — четыре.
График также получил компактную раскладку в панели. Факты, суммы, цвета
и переходы к основаниям не менялись.
В браузере проверены карточки без переполнения в панели 400 px, две колонки
в отчёте 952 px, график, прокрутка таблицы и открытие оснований.
Typecheck, 13 профильных тестов и production build пройдены. Общий набор:
90 passed / прежние 46 failed (имена сверены); lint — прежние 2 ошибки.
Web пересобран и обновлён для локального просмотра.

## История слева и старт по центру — решение пользователя 06.09.2026

Перенести историю проверок под основной пункт левой навигации и сохранять её
доступной внутри проекта; стартовое поле расположить в центре свободной области.
На мобильном история открывается отдельно. Убрать кнопку «Материалы» из шапки,
сохранив контекстное открытие оснований из чата и управление компаниями, включая
случай лимита 20. Это уточнение заменяет прежнюю композицию S1/S2 из UI baseline;
агента, данные и протоколы не менять.

Реализовано: общая история с поиском и выделением текущей проверки, центральный
стартовый composer, мобильное меню с Escape и возвратом фокуса. Кнопка «Материалы»
убрана; основания открываются прежними обработчиками, а «ещё N» открывает список
компаний даже при лимите 20. Создание проверки и отправка вопроса не менялись.
В браузере проверены 390/720/1024/1440 px, длинная история, поиск, переходы,
список компаний и основание загруженного отчёта. Нового прогона модели не было.
Typecheck/build пройдены; общий набор — 89 passed / прежние 46 failed, без новых
падений (сверены имена); lint содержит прежние 2 ошибки вне этого изменения.
Временный стенд удалён, web пересобран для локального просмотра.

Уточнение пользователя: убрать подсказку Enter/Shift+Enter на главной и в чате,
не меняя клавиатурную отправку. Надпись «Альфа-Бизнес» увеличена; в красном
квадрате — белая «А» из уже подключённой библиотеки иконок Alfa, включая mobile.
Проверены typecheck, 20 профильных тестов, Docker-сборка и внешний вид в браузере.
По следующему запросу убрана подпись «Демонстрационная версия» из боковой панели.
Возврат на главную перенесён с кнопки «Все проверки» в шапке на логотип
«Альфа-Бизнес» в боковой и мобильной навигации. Лишняя колонка и разделитель
шапки убраны; логотип закрывает меню истории при переходе.
Проверены возврат в браузере, typecheck, 8 профильных тестов и Docker-сборка.

## Шапка проверки — 06.09.2026

По скриншоту пользователя упорядочена только верхняя панель S2: название
и сохранение объединены, длинное название не сдвигает действия; совпадающее
название чата заменяется видимой подписью «Чат проверки» без переименования
в БД. Полные названия доступны в подсказках, меню и доступных именах.
Кнопки, строка компаний и статус оформлены единообразно; мобильная компоновка
проверена в браузере на 390/700/1024/1440 px. Основной порядок блоков, API,
агент и отчёты не менялись. Добавлены 4 проверки шапки; typecheck/build пройдены.
Общий набор: 86 passed / прежние 46 failed, в lint те же 2 сторонние ошибки.
Временный стенд просмотра удалён. Для просмотра пересобран web в Docker.
...
## Граница роли агента — 07.09.2026

После живого примера с `ls`, созданием файла и кодом универсальные возможности
Deep Agents отключены штатным `HarnessProfile`: модель больше не видит файловые
tools, `execute` и `task`, а стандартный general-purpose subagent не создаётся.
Виртуальная рабочая папка и её путь удалены из контекста. System prompt явно
фиксирует единственную роль — проверка контрагентов — и короткий отказ от кода,
файлов, команд и иных посторонних задач. Проверена итоговая сборка prompt:
Deep Agents 0.7.13 для текущего OpenAI-compatible `deepseek-v4-flash` не добавляет
постоянных универсальных инструкций; остаются только служебные middleware без
prompt-текста. Ruff, format и mypy пройдены; 45 тестов пройдены, 14 PostgreSQL
skipped, один socket-тест исключён из sandbox-прогона (bind запрещён средой).
