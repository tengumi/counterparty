# Альфа-Бизнес — проверка контрагентов
...
Решение разработано в рамках хакатона [AI Talent Hub](https://ai.itmo.ru/):
кейс «Агент и MCP-сервер для проверки контрагента», команда «viber's». Проект был отмечен за продуктовую проработанность.

[Презентация проекта](assets/presentation.pdf)

## Возможности

- Проверка компании по ИНН в формате сохраняемого диалога.
- Ответы агента с кликабельными основаниями из отчёта.
- Сравнение от 2 до 20 контрагентов по детерминированным критериям.
- Отдельное отображение рисков, отсутствующих данных и внешних оценок.
- Изолированная проектная память и MCP-доступ к сведениям отчётов.

Стек: React, TypeScript, FastAPI, LangGraph, FastMCP и PostgreSQL. Весь контур
...
![Стартовый экран проверки контрагента](assets/01-start.png)

### 2. Добавление компаний

![Добавление компаний в проверку](assets/02-companies.png)

### 3. Ответ агента с проверяемыми основаниями

![Ответ агента с основаниями](assets/03-agent-answer.png)

### 4. Отчёт по контрагенту

![Ключевые сигналы и данные отчёта](assets/04-report.png)
...
- `artifacts/contractors_audit.snapshot.json` — основной разрешённый
  mock-источник; создавать отдельную «очищенную» fixture не требуется.

## Источники истины

При расхождении документов используется следующий приоритет:

1. `docs/Specs/` версии 1.1 — продуктовые и системные требования;
2. `docs/architecture.md` — краткая карта архитектуры;
3. `artifacts/` — описание кейса и исходные mock-данные.
...
Vite проксирует `/api/v1` на `localhost:8000`, поэтому
...
В `services/agent` pytest проверяет общие контракты и инфраструктуру. Поведение
агента дополнительно проверяется ручным живым прогоном.
...
liveness, а не готовность полного агентного сценария.
