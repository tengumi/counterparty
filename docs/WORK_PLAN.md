...
Исключение по решению пользователя 06.09.2026: в `services/agent` не писать
хрупкие автотесты конкретных ответов, промптов и сценарных действий модели.
Сохраняются общие проверки контрактов, инфраструктуры, изоляции и evidence refs;
поведение проверяется ручными живыми прогонами. Это правило действует и для
агентных задач ниже; границы описаны в `services/agent/AGENTS.md`.
...
| AG-07 | Durable публичная проекция разговора: агент пишет финальный `PublicAgentState` (messages/activities) в `workspace` и отдаёт по RPC; ui_api-эндпоинт `…/conversation` проксирует вместо пустой проекции. Ревизии по `agent_runs.last_public_revision`, граница I7 сохраняется | AG-04 | done (06.09.2026: агент на терминальном переходе фолдит event-log в `PublicAgentState` и пишет его в новую JSONB-колонку `workspace.agent_runs.public_projection` на fenced owner-connection; ui_api `…/conversation` читает её своим `AgentRunReadRepository` (без нового HTTP-стыка ui_api→agent), lifecycle/`revision`/`context_version` остаются из авторитетных строк; миграция `0007`; storage 88, agent 74, ui_api 72, migrations 19 зелёные + живой прогон компании А через proxy отдал полную grounded-историю) |
...
| QA-04 | Ручные живые прогоны сценариев Specs 08 §5; без хрупких автоматических проверок ответов и сценарных действий модели (решение пользователя 06.09.2026) | AG-04, AG-06 | todo |
